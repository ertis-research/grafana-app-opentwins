// src/components/ConnectionForm/components/FormSections/SourcesSection.tsx
import React, { Fragment, FormEvent } from 'react'
import { Alert, Button, CodeEditor, Field, Input, Select } from '@grafana/ui'
import { SelectableValue } from '@grafana/data'
import { SourceData } from 'utils/interfaces/connections'
...
export const SourcesSection: React.FC<Props> = ({ sources, qosOptions, pmOptions, onInputChange, onSelectChange, onBlurOthers, onRemove }) => {
    return (
        <Fragment>
            {sources.map((s: SourceData, idx: number) => (
                <Fragment key={idx}>
                    ...
                    <Field label="Payload mapping" description="Mapper that will process the messages. Default is Ditto protocol.">
                        <Select options={pmOptions} value={s.payloadMapping} onChange={(e) => onSelectChange(e, keys.sources, keys.payloadMapping, idx)} />
                    </Field>
                    <Field label="Other configurations" description="Add any other Eclipse Ditto settings you need in this section. This will be mixed in with the rest of the data">
                        <CodeEditor value={s.others} language='json' height={100}
                            onBlur={(c) => onBlurOthers(c, idx, keys.sources)}
                            showLineNumbers={true} showMiniMap={false}
                            monacoOptions={{ formatOnPaste: true, formatOnType: true }}
                        />
                    </Field>
                    <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                        <Button variant='destructive' icon='trash-alt' onClick={() => onRemove(keys.sources, idx)}>Delete</Button>
                    </div>
                </Fragment>
            ))}
        </Fragment>
